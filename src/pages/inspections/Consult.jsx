...
import { getAllInspecoes, getEnderecoById } from "../../services/api";
...
const Consult = () => {
    ...
    useEffect(() => {
        ...
        if (filterTipo) {
            filtered = filtered.filter(
                (inspecao) => inspecao.tipo === filterTipo
            );
        }
    ...
    const fetchInspecoes = async () => {
        try {
            setLoading(true);
            setError(null);
            const data = await getAllInspecoes();

            // Para cada inspeção, buscar os dados completos do endereço
            const inspecoesComEndereco = await Promise.all(
                data.map(async (inspecao) => {
                    try {
                        if (inspecao.enderecoId) {
                            const enderecoCompleto = await getEnderecoById(
                                inspecao.enderecoId
                            );
                            return {
                                ...inspecao,
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex flex-wrap gap-4 mt-4 mb-6">
                                ...
                                >
                                    ...
                                    <option value="Animal peçonhento">
                                        Animal peçonhento
                                    </option>
                                    <option value="Foco de mosquito">
                                        Foco de mosquito
                                    </option>
                                    <option value="Água parada">
                                        Água parada
                                    </option>
                                    <option value="Lixo acumulado">
                                        Lixo acumulado
                                    </option>
